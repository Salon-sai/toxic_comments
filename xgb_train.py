...
def main(args):
    ...
    with timer("Scoring Light GBM"):
        ...
        # train_xgbset = xgb.DMatrix(data=all_train_features)
        ...
        for label_name in label_names:
            ...
            # train_xgbset.set_label(train_target.values)
            ...
            stack_preds[label_name + "_lgb_oof"] = preds
    ...
    with timer("Predicting probabilities"):
        for label_name in label_names:
            with timer("Predicting probabilities for %s" % label_name):
                train_target = train_data[label_name].values
                # train_lgbset.set_label(train_target.values)
                train_set = xgb.DMatrix(all_train_features, train_target)
...
def valid_train_lgm(params, train_features, train_data, label_name, n_split=5):
    folds = KFold(n_splits=n_split)
    train_target = train_data[label_name].values
    # train_features.set_label(train_target.values)

    n_estimators = 800
    class_pred = np.zeros(train_data.shape[0])

    indices = np.arange(len(train_data))
    for n_fold, (train_indices, valid_indices) in enumerate(folds.split(indices)):
        watchlist = [(train_features[train_indices], train_target[train_indices]),
                     (train_features[valid_indices], train_target[valid_indices])]

        model = xgb.XGBClassifier(max_depth=3, nthread=4, n_estimators=n_estimators, learning_rate=0.05,
                                  min_child_weight=4, subsample=0.7, colsample_bytree=0.7, random_state=1)

        model.fit(train_features[train_indices], train_target[train_indices],
                  early_stopping_rounds=50,
                  verbose=False,
                  eval_metric="auc",
                  eval_set=watchlist)
        class_pred[valid_indices] = model.predict(train_features[valid_indices])
        score = roc_auc_score(train_target[valid_indices], class_pred[valid_indices])
